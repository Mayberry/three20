...
- (TTStyle*)styleWithSelector:(NSString*)selector forState:(UIControlState)state {
    ...
    BOOL hasArguments = [components count] > 0;
    
    if (!style) {
        SEL sel = (!hasArguments)
        ? NSSelectorFromString(selector) 
        : NSSelectorFromString([NSString stringWithFormat:@"%@:", [components objectAtIndex:0]]);
        
        if ([self respondsToSelector:sel]) {
            style = (!hasArguments)
            ? [self performSelector:sel withObject:(id)state] 
            : [self performSelector:sel withObject:[components objectAtIndex:1]];
